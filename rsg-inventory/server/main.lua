...
CreateThread(function()
    while true do
        Wait(1000)
        if ItemsLoaded then
            TriggerClientEvent('rsg-inventory:client:updateItemData', -1, Config.ItemPlacer)
        end
    end
end)

CreateThread(function()
    TriggerEvent('rsg-inventory:server:getItems')
        ItemsLoaded = true
end)

RegisterServerEvent('rsg-inventory:server:getItems')
AddEventHandler('rsg-inventory:server:getItems', function()
    local result = MySQL.query.await('SELECT * FROM item_placed')
    if not result[1] then return end
    for i = 1, #result do
        local itemData = json.decode(result[i].properties)
        print('loading ' .. itemData.itemtype .. ' item with ID: ' .. itemData.id)
        table.insert(Config.ItemPlacer, itemData)
    end
end)

RegisterServerEvent('rsg-inventory:server:removeitemdropp')
AddEventHandler('rsg-inventory:server:removeitemdropp', function(item, amount)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    Player.Functions.RemoveItem(item, amount)
    TriggerClientEvent('inventory:client:ItemBox', src, RSGCore.Shared.Items[item], 'remove')
    TriggerClientEvent('ox_lib:notify', src, {title = 'You Placed ' .. item, type = 'success',icon = 'fa-solid fa-box', iconAnimation = 'shake', duration = 7000 })

end)

RegisterServerEvent('rsg-inventory:server:pickupItem')
AddEventHandler('rsg-inventory:server:pickupItem', function(itemId)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    for i, item in ipairs(Config.ItemPlacer) do
        if item.id == itemId then 
            local success = Player.Functions.AddItem(item.itemtype, 1) -- Adjust the amount as needed
            if success then
                TriggerClientEvent('inventory:client:ItemBox', src, RSGCore.Shared.Items[item.itemtype], 'add')
                TriggerClientEvent('ox_lib:notify', src, {title = 'You picked up ' .. item.itemtype, type = 'success',icon = 'fa-solid fa-box', iconAnimation = 'shake', duration = 7000 })
                table.remove(Config.ItemPlacer, i)
                TriggerClientEvent('rsg-inventory:client:removeItemObject', -1, itemId)
                TriggerEvent('rsg-inventory:server:DropRemoved', itemId)
            else
                TriggerClientEvent('ox_lib:notify', src, {title = 'Inventory full!', type = 'error',icon = 'fa-solid fa-box', iconAnimation = 'shake', duration = 7000 })
                print("Failed to add item to inventory. Item type: " .. item.itemtype) -- Debug print
            end
            return
        end
    end
    TriggerClientEvent('ox_lib:notify', src, {title = 'Item not found!', type = 'error',icon = 'fa-solid fa-box', iconAnimation = 'shake', duration = 7000 })
end)


RegisterServerEvent('rsg-inventory:server:DropRemoved')
AddEventHandler('rsg-inventory:server:DropRemoved', function(itemid)
    local result = MySQL.query.await('SELECT * FROM item_placed')
    ...
    for i = 1, #result do
        ...
        if itemData.id == itemid then
            MySQL.Async.execute('DELETE FROM item_placed WHERE id = @id', { ['@id'] = result[i].id })
            for k, v in pairs(Config.ItemPlacer) do
                if v.id == itemid then
                    table.remove(Config.ItemPlacer, k)
                end
            end
        end
    end
...
-- Place item
RegisterServerEvent('rsg-inventory:server:placeitem')
AddEventHandler('rsg-inventory:server:placeitem', function(outputitem, prophash, position, heading)
    local src = source
    local placeid = math.random(111111, 999999)
    local Player = RSGCore.Functions.GetPlayer(src)
    ...
    local ItemData =
    {
        ...
        h = heading,
        hash = prophash,
        dropper = Player.PlayerData.citizenid,
        droptime = os.time()
    }

    local ItemCount = 0

    for _, v in pairs(Config.ItemPlacer) do
        if v.dropper == Player.PlayerData.citizenid then
            ItemCount = ItemCount + 1
        end
    end

    if ItemCount >= Config.MaxItemCount then
            TriggerClientEvent('ox_lib:notify', src, {title = 'Max amount of items down, max of ' ..Config.MaxItemCount..' items', type = 'error', duration = 7000 })
    else
        table.insert(Config.ItemPlacer, ItemData)
        TriggerEvent('rsg-inventory:server:saveItem', ItemData, placeid, citizenid)
        TriggerEvent('rsg-inventory:server:updatePlants')
    end
end)

RegisterServerEvent('rsg-inventory:server:saveItem')
AddEventHandler('rsg-inventory:server:saveItem', function(data, itemid, citizenid)
    local datas = json.encode(data)

    MySQL.Async.execute(
        'INSERT INTO item_placed (properties, itemid, citizenid) VALUES (@properties, @itemid, @citizenid)',
        {
            ['@properties'] = datas,
            ['@itemid'] = itemid,
            ['@citizenid'] = citizenid
...
RSGCore.Functions.CreateCallback('rsg-inventory:server:createDrop', function(source, cb)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    if not Player then
        cb(false)
    ...
    for itemToDrop, prop in pairs(ItemDropped) do
        local hasItem = Player.Functions.GetItemByName(itemToDrop)
        if hasItem then
            exports['rsg-inventory']:CloseInventory(src)
            TriggerClientEvent('rsg-inventory:client:predropitemprop', src, itemToDrop, prop, itemToDrop)
            break
        end
    end
    cb(true)
        ...
        if Player.PlayerData.money.cash >= price then
            if sinvtype == 'player' then
                TriggerClientEvent('ox_lib:notify', source,
                    { title = 'This shop do not buy your items!', type = 'error', duration = 5000 })
                cb(false)
            else
            ...
            TriggerClientEvent('ox_lib:notify', source,
                { title = 'You do not have enough money', type = 'error', duration = 5000 })
            cb(false)
        end
    else
        TriggerClientEvent('ox_lib:notify', source,
...
RegisterNetEvent('rsg-inventory:server:SetInventoryData',
